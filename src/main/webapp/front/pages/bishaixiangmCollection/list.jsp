            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">比赛项目收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="bishaixiangmName" id="bishaixiangmName" placeholder="比赛项目" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="bishaixiangmTypes" id="bishaixiangmTypes" lay-filter="bishaixiangmTypes">
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../bishaixiangm/detail.jsp?id='+item.bishaixiangmId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.bishaixiangmPhoto?item.bishaixiangmPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../bishaixiangm/detail.jsp?id='+item.bishaixiangmId)"><span>{{item.bishaixiangmName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.bishaixiangmNewMoney"><span style="color: red;">{{item.bishaixiangmNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteBishaixiangmCollection(item.id)">删除 {{item.bishaixiangmName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            bishaixiangmCollectionTypes: 1,//类型
            limit: 10,
        ...
        filters: {
            //把 bishaixiangmCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            bishaixiangmCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteBishaixiangmCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('bishaixiangmCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#bishaixiangmName').val() != null && jquery('#bishaixiangmName').val() != "" && jquery('#bishaixiangmName').val() != "null" ) {
                param['bishaixiangmName'] = jquery('#bishaixiangmName').val();
            }
                     
            var bishaixiangmTypes = $("#bishaixiangmTypes").find("option:selected").val();
            if (bishaixiangmTypes != null && bishaixiangmTypes != "" && bishaixiangmTypes != "null") {
                            param['bishaixiangmTypes'] = bishaixiangmTypes;
                        }
                                                                                                                        
            // 当前表
                                                    // 获取列表数据
            http.request('bishaixiangmCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function bishaixiangmTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=bishaixiangm_types", 'get', {}, function (res) {
               if(res.code == 0){
