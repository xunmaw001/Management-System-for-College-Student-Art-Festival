...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的比赛项目报名模块 </p>
    </div>
    <!-- 标题 -->
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.bishaixiangmName}}</td>
                            <td style="width: 80px;">{{item.bishaixiangmValue}}</td>
                            <td style="width: 60px;">{{item.bishaixiangmNumber}}</td>
                            <td style="width: 60px;">{{item.bishaixiangmTime}}</td>
                            <td style="width: 250px;">
                                <img :src="item.bishaixiangmPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 bishaixiangmOrderContent 字段去除无用字符 如果字段长度大于60  就截取到60
            bishaixiangmOrderContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            //列表查询
            ,pageList(bishaixiangmOrderTypes) {
                this.bishaixiangmOrderTypes = bishaixiangmOrderTypes;
                // 获取列表数据
                layui.http.request('bishaixiangmOrder/list', 'get', {
                    page: 1,
                    limit: this.limit,
                    bishaixiangmOrderTypes: vue.bishaixiangmOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('bishaixiangmOrder/list', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    bishaixiangmOrderTypes: this.bishaixiangmOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                // 获取商品详情信息
                layui.http.requestJson(`bishaixiangmOrder/pay`, 'PPST',{
                    id:id
                }, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
            ...
            ,refund(id) {
                // 获取商品详情信息
                layui.http.request(`bishaixiangmOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        // 获取列表数据
        http.request('bishaixiangmOrder/list', 'get',{
            page: 1,
            limit: limit,
            bishaixiangmOrderTypes: vue.bishaixiangmOrderTypes
        }, function(res) {
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('bishaixiangmOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            bishaixiangmOrderType: vue.bishaixiangmOrderType
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
