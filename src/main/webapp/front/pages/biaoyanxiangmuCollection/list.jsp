            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">表演项目收藏展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">表演项目</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="biaoyanxiangmuName" id="biaoyanxiangmuName" placeholder="表演项目" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="biaoyanxiangmuTypes" id="biaoyanxiangmuTypes" lay-filter="biaoyanxiangmuTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in biaoyanxiangmuTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../biaoyanxiangmu/detail.jsp?id='+item.biaoyanxiangmuId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.biaoyanxiangmuPhoto?item.biaoyanxiangmuPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../biaoyanxiangmu/detail.jsp?id='+item.biaoyanxiangmuId)"><span>{{item.biaoyanxiangmuName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.biaoyanxiangmuNewMoney"><span style="color: red;">{{item.biaoyanxiangmuNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteBiaoyanxiangmuCollection(item.id)">删除 {{item.biaoyanxiangmuName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            biaoyanxiangmuCollectionTypes: 1,//类型
            limit: 10,
        ...
        filters: {
            //把 biaoyanxiangmuCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            biaoyanxiangmuCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteBiaoyanxiangmuCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('biaoyanxiangmuCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#biaoyanxiangmuName').val() != null && jquery('#biaoyanxiangmuName').val() != "" && jquery('#biaoyanxiangmuName').val() != "null" ) {
                param['biaoyanxiangmuName'] = jquery('#biaoyanxiangmuName').val();
            }
                     
            var biaoyanxiangmuTypes = $("#biaoyanxiangmuTypes").find("option:selected").val();
            if (biaoyanxiangmuTypes != null && biaoyanxiangmuTypes != "" && biaoyanxiangmuTypes != "null") {
                            param['biaoyanxiangmuTypes'] = biaoyanxiangmuTypes;
                        }
                                                                                                                        
            // 当前表
                                                    // 获取列表数据
            http.request('biaoyanxiangmuCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function biaoyanxiangmuTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=biaoyanxiangmu_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.biaoyanxiangmuTypesList = res.data.list;
               }
           });
       }
