...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的表演项目报名模块 </p>
    </div>
    <!-- 标题 -->
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.biaoyanxiangmuName}}</td>
                            <td style="width: 80px;">{{item.biaoyanxiangmuValue}}</td>
                            <td style="width: 60px;">{{item.biaoyanxiangmuNumber}}</td>
                            <td style="width: 60px;">{{item.biaoyanxiangmuTime}}</td>
                            <td style="width: 250px;">
                                <img :src="item.biaoyanxiangmuPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 biaoyanxiangmuOrderContent 字段去除无用字符 如果字段长度大于60  就截取到60
            biaoyanxiangmuOrderContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            //列表查询
            ,pageList(biaoyanxiangmuOrderTypes) {
                this.biaoyanxiangmuOrderTypes = biaoyanxiangmuOrderTypes;
                // 获取列表数据
                layui.http.request('biaoyanxiangmuOrder/list', 'get', {
                    page: 1,
                    limit: this.limit,
                    biaoyanxiangmuOrderTypes: vue.biaoyanxiangmuOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('biaoyanxiangmuOrder/list', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    biaoyanxiangmuOrderTypes: this.biaoyanxiangmuOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                // 获取商品详情信息
                layui.http.requestJson(`biaoyanxiangmuOrder/pay`, 'PPST',{
                    id:id
                }, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
            ...
            ,refund(id) {
                // 获取商品详情信息
                layui.http.request(`biaoyanxiangmuOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        // 获取列表数据
        http.request('biaoyanxiangmuOrder/list', 'get',{
            page: 1,
            limit: limit,
            biaoyanxiangmuOrderTypes: vue.biaoyanxiangmuOrderTypes
        }, function(res) {
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('biaoyanxiangmuOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            biaoyanxiangmuOrderType: vue.biaoyanxiangmuOrderType
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
